...
def record_invoice(invoice_file: TextIO,
    ...
    :return: The position of the start of the last line of the file.
            This can be used in subsquent calls to `record_invoice`
    """
    
    invoice_file.seek(last_line_ptr, SEEK_SET)  # file.seek(offset, whence)
    last_row = ""
    ...
    last_line_ptr = invoice_file.tell() #"Where is the file pointer right now?"
...
with open(data_file, "r+") as invoices: # r+ = read + write
   last_line=record_invoice(invoices, "ACME ROADRUNNER", 136.9)
   last_line=record_invoice(invoices, "Squirrel stroage", 69.0,last_line)
# Test code:
